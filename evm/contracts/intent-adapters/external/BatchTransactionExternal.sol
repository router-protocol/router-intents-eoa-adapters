 ...
 */
contract BatchTransactionExternal is
    Basic,
    AccessControl,
    ReentrancyGuard,
    IERC721Receiver
{
    ...
    bool private _isBatchExternalLive;
    // user -> token array
    ...
    /**
     * @notice function to return the contract state.
     */

    function isLive() public view virtual returns (bool) {
        return _isBatchExternalLive;
    }

    /**
     * @notice function to set Batch as Live.
     * @param __isLive Boolean.
     */
    function setContractLive(
        bool __isLive
    ) external onlyRole(SETTER_ROLE) {
        _isBatchExternalLive = __isLive;
    }
    ...
    modifier onlyNitro() {
     ...
     /**
     * @notice modifier to ensure that batch trnx is only called when live
     */
    modifier onlyWhenLive() {
        require(
            _isBatchExternalLive == true,
            "batch external is not livex"
        );
        _;
    }
