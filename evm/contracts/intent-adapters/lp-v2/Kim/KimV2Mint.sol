...
import {KimHelpers} from "./KimHelpers.sol";

/**
 * @title KimV2Mint
 * @author Yashika Goyal
 * @notice Adding liquidity on Kim Protocol.
 */

contract KimV2Mint is RouterIntentEoaAdapterWithoutDataProvider, KimHelpers {
    ...
    function _mint(
        IKimRouter.KimSupplyData memory _mintParams
    ) internal returns (uint256 amountA, uint256 amountB, uint256 liqAmount) {
            (amountA, amountB, liqAmount) = kimRouter.addLiquidity(
                _mintParams.tokenA,
                _mintParams.tokenB,
    ...
    function parseInputs(
        ...
        public
        pure
        returns (IKimRouter.KimSupplyData memory)
    {
        return
            abi.decode(data, (IKimRouter.KimSupplyData));
    }
