...
import {TrisolarisHelpers} from "./TrisolarisHelpers.sol";

/**
 * @title TrisolarisMint
 * @author Yashika Goyal
 * @notice Adding liquidity on Trisolaris.
 */

contract TrisolarisMint is RouterIntentEoaAdapterWithoutDataProvider, TrisolarisHelpers {
    using SafeERC20 for IERC20;

    constructor(
        address __native,
        address __wnative,
        address __solarisRouter
    )
        RouterIntentEoaAdapterWithoutDataProvider(__native, __wnative)
        TrisolarisHelpers(__solarisRouter)
    ...
    function execute(
        ...
        ITrisolarisRouter.TrisolarisSupplyData
    ...
    function _mint(
        ITrisolarisRouter.TrisolarisSupplyData memory _mintParams
    ) internal returns (uint256 amountA, uint256 amountB, uint256 liqAmount) {
            (amountA, amountB, liqAmount) = solarisRouter.addLiquidity(
                _mintParams.tokenA,
                _mintParams.tokenB,
    ...
    function parseInputs(
        ...
        public
        pure
        returns (ITrisolarisRouter.TrisolarisSupplyData memory)
    {
        return
            abi.decode(data, (ITrisolarisRouter.TrisolarisSupplyData));
    }
