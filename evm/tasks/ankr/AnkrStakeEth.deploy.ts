...
import {
  ...
  DEPLOY_ANKR_STAKE_ETH_ADAPTER,
  DEXSPAN,
  NATIVE,
  VERIFY_ANKR_STAKE_ETH_ADAPTER,
  WNATIVE,
} from "../constants";
...
const contractName: string = CONTRACT_NAME.AnkrStakeEth;

task(DEPLOY_ANKR_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_ANKR_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_ANKR_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
