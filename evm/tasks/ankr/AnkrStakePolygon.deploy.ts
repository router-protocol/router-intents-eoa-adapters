...
const contractName: string = CONTRACT_NAME.AnkrStakePolygon;

task(DEPLOY_ANKR_STAKE_POLYGON_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_ANKR_STAKE_POLYGON_ADAPTER);
    }
  });

task(VERIFY_ANKR_STAKE_POLYGON_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
