...
import { BENQI_S_AVAX } from "./constants";

const contractName: string = CONTRACT_NAME.BenqiStakeAvax;

task(DEPLOY_BENQI_STAKE_AVAX_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_BENQI_STAKE_AVAX_ADAPTER);
    }
  });

task(VERIFY_BENQI_STAKE_AVAX_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
