...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_AERODROME_MINT_ADAPTER,
  NATIVE,
  VERIFY_AERODROME_MINT_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { AERODROME_ROUTER } from "./constants";

const contractName: string = CONTRACT_NAME.AerodromeMint;
const contractType = ContractType.LP;

task(DEPLOY_AERODROME_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_AERODROME_MINT_ADAPTER);
    }
  });

task(VERIFY_AERODROME_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
