...
const contractName: string = CONTRACT_NAME.AnkrStakeFantom;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_ANKR_STAKE_FANTOM_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_ANKR_STAKE_FANTOM_ADAPTER);
    }
  });

task(VERIFY_ANKR_STAKE_FANTOM_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
