...
import { BENQI_QI_USDC } from "./constants";

const contractName: string = CONTRACT_NAME.BenqiSupply;
const contractType = ContractType.LendingBorrowing;

task(DEPLOY_BENQI_SUPPLY_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_BENQI_SUPPLY_ADAPTER);
    }
  });

task(VERIFY_BENQI_SUPPLY_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
