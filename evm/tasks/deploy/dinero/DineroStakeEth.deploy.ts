...
const contractName: string = CONTRACT_NAME.DineroStakeEth;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_DINERO_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_DINERO_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_DINERO_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
