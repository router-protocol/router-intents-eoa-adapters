...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_EIGENLAYER_AFFINE_ADAPTER,
  NATIVE,
  VERIFY_EIGENLAYER_AFFINE_ADAPTER,
  WNATIVE,
} from "../../constants";
...
const contractName: string = CONTRACT_NAME.EigenLayerAffine;
const contractType = ContractType.Staking;

task(DEPLOY_EIGENLAYER_AFFINE_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_EIGENLAYER_AFFINE_ADAPTER);
    }
  });

task(VERIFY_EIGENLAYER_AFFINE_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
