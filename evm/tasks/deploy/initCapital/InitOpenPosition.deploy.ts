...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_INIT_OPEN_POSITION_ADAPTER,
  NATIVE,
  VERIFY_INIT_OPEN_POSITION_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { UNIVERSAL_ROUTER } from "./constants";

const contractName: string = CONTRACT_NAME.InitOpenPosition;
const contractType = ContractType.Others;

task(DEPLOY_INIT_OPEN_POSITION_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_INIT_OPEN_POSITION_ADAPTER);
    }
  });

task(VERIFY_INIT_OPEN_POSITION_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
