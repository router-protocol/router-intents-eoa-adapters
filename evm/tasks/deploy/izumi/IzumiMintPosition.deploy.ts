...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_IZUMI_MINT_ADAPTER,
  NATIVE,
  VERIFY_IZUMI_MINT_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { LIQUIDITY_MANAGER } from "./constants";

const contractName: string = CONTRACT_NAME.IzumiMint;
const contractType = ContractType.LP;

task(DEPLOY_IZUMI_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_IZUMI_MINT_ADAPTER);
    }
  });

task(VERIFY_IZUMI_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
