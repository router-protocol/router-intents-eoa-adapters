...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_KIM_V2_MINT_ADAPTER,
  NATIVE,
  VERIFY_KIM_V2_MINT_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { KIM_ROUTER } from "./constants";

const contractName: string = CONTRACT_NAME.KimV2Mint;
const contractType = ContractType.LP;

task(DEPLOY_KIM_V2_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_KIM_V2_MINT_ADAPTER);
    }
  });

task(VERIFY_KIM_V2_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
