...
import { LAYER_BANK_CORE, USDC, WSTETH } from "./constants";

const contractName: string = CONTRACT_NAME.LayerBankSupplyScroll;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_LAYER_BANK_SUPPLY_SCROLL_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_LAYER_BANK_SUPPLY_SCROLL_ADAPTER);
    }
  });

task(VERIFY_LAYER_BANK_SUPPLY_SCROLL_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
