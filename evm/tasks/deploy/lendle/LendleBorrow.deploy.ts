...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_LENDLE_BORROW_ADAPTER,
  NATIVE,
  VERIFY_LENDLE_BORROW_ADAPTER,
  WNATIVE,
} from "../../constants";
...
const contractName: string = CONTRACT_NAME.LendleBorrow;
const contractType = ContractType.LendingBorrowing;

task(DEPLOY_LENDLE_BORROW_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_LENDLE_BORROW_ADAPTER);
    }
  });

task(VERIFY_LENDLE_BORROW_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
