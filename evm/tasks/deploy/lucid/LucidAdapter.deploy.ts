...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_LUCID_ADAPTER,
  NATIVE,
  VERIFY_LUCID_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { FIXED_EXPIRY_TELLER, FIXED_TERM_TELLER } from "./constants";

const contractName: string = CONTRACT_NAME.LucidAdapter;
const contractType = ContractType.FPA;

task(DEPLOY_LUCID_ADAPTER)
  ...
  .setAction(async function (
    ...
    const instance = await factory.deploy(
      NATIVE,
      WNATIVE[env][network],
      FIXED_EXPIRY_TELLER[network],
      FIXED_TERM_TELLER[network]
    );
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_LUCID_ADAPTER);
    }
  });

task(VERIFY_LUCID_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
