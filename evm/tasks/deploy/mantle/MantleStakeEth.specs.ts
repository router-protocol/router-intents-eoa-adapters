...
import { MANTLE_STAKING, METH } from "./constants";

const contractName: string = CONTRACT_NAME.MantleStakeEth;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_MANTLE_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_MANTLE_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_MANTLE_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
