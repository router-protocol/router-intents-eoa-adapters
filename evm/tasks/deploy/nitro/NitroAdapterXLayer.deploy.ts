...
import {
  ...
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_NITRO_XLAYER_ADAPTER,
  DEXSPAN,
  NATIVE,
  VERIFY_NITRO_XLAYER_ADAPTER,
  WNATIVE,
} from "../../constants";
...
task(DEPLOY_NITRO_XLAYER_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_NITRO_XLAYER_ADAPTER);
    }
  });

task(VERIFY_NITRO_XLAYER_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
