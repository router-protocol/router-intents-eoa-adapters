...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_PARIFI_FAIRSALE_ADAPTER,
  NATIVE,
  VERIFY_PARIFI_FAIRSALE_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { PARIFI_FAIRSALE_CONTRACT } from "./constants";

const contractName: string = CONTRACT_NAME.ParifiFairsale;
const contractType = ContractType.Others;

task(DEPLOY_PARIFI_FAIRSALE_ADAPTER)
  ...
  .setAction(async function (
    ...
    const instance = await factory.deploy(
      NATIVE,
      WNATIVE[env][network],
      PARIFI_FAIRSALE_CONTRACT[network]
    );
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_PARIFI_FAIRSALE_ADAPTER);
    }
  });

task(VERIFY_PARIFI_FAIRSALE_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
