...
import { M_PENDLE_CONVERTER, M_PENDLE_RECEIPT_TOKEN, PENDLE_TOKEN } from "./constants";

const contractName: string = CONTRACT_NAME.PenpieStakePendle;
const contractType = ContractType.Staking;

task(DEPLOY_PENPIE_STAKE_PENDLE_ADAPTER)
  ...
  .setAction(async function (
    ...
    const instance = await factory.deploy(
      NATIVE,
      WNATIVE[env][network],
      PENDLE_TOKEN[network],
      M_PENDLE_RECEIPT_TOKEN[network],
      M_PENDLE_CONVERTER[network]
    );
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_PENPIE_STAKE_PENDLE_ADAPTER);
    }
  });

task(VERIFY_PENPIE_STAKE_PENDLE_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
