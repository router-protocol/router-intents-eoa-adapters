...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_WEN_SWAPPER_ADAPTER,
  NATIVE,
  VERIFY_WEN_SWAPPER_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { WEN_FOUNDRY } from "./constants";

const contractName: string = CONTRACT_NAME.WenSwapper;
const contractType = ContractType.Swap;

task(DEPLOY_WEN_SWAPPER_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_WEN_SWAPPER_ADAPTER);
    }
  });

task(VERIFY_WEN_SWAPPER_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
