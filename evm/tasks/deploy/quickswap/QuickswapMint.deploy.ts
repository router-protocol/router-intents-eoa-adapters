...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_QUICKSWAP_MINT_ADAPTER,
  NATIVE,
  VERIFY_QUICKSWAP_MINT_ADAPTER,
  WNATIVE,
} from "../../constants";
...
const contractName: string = CONTRACT_NAME.QuickswapMint;
const contractType = ContractType.LP;

task(DEPLOY_QUICKSWAP_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_QUICKSWAP_MINT_ADAPTER);
    }
  });

task(VERIFY_QUICKSWAP_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
