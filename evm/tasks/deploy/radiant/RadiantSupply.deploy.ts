...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_RADIANT_SUPPLY_ADAPTER,
  NATIVE,
  VERIFY_RADIANT_SUPPLY_ADAPTER,
  WNATIVE,
} from "../../constants";
...
const contractName: string = CONTRACT_NAME.RadiantSupply;
const contractType = ContractType.LendingBorrowing;

task(DEPLOY_RADIANT_SUPPLY_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_RADIANT_SUPPLY_ADAPTER);
    }
  });

task(VERIFY_RADIANT_SUPPLY_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
