...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_REDACTED_STAKE_ETH_ADAPTER,
  NATIVE,
  VERIFY_REDACTED_STAKE_ETH_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { PX_ETH, PIREX_ETH } from "./constants";

const contractName: string = CONTRACT_NAME.RedactedStakeEth;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_REDACTED_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_REDACTED_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_REDACTED_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
