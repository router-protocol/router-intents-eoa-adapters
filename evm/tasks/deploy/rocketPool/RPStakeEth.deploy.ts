...
import { R_ETH, ROCKET_DEPOSIT_POOL } from "./constants";

const contractName: string = CONTRACT_NAME.RPStakeEth;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_ROCKET_POOL_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_ROCKET_POOL_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_ROCKET_POOL_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
