...
import { SHARED_STAKE_WSGETH, SHARED_STAKE_DEPOSIT_MINTER } from "./constants";

const contractName: string = CONTRACT_NAME.SharedStakeEth;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_SHARED_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    const instance = await factory.deploy(
      NATIVE,
      WNATIVE[env][network],
      SHARED_STAKE_WSGETH[network],
      SHARED_STAKE_DEPOSIT_MINTER[network]
    );
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_SHARED_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_SHARED_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
