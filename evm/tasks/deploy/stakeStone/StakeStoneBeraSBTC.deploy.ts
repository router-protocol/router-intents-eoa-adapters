...
import { BERA_SBTC, BERA_SBTC_VAULT } from "./constants";

const contractName: string = CONTRACT_NAME.StakeStoneBeraSBTC;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_STAKESTONE_BERA_SBTC_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_STAKESTONE_BERA_SBTC_ADAPTER);
    }
  });

task(VERIFY_STAKESTONE_BERA_SBTC_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
