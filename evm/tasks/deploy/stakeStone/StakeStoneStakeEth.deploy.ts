...
import { STONE, STONE_VAULT } from "./constants";

const contractName: string = CONTRACT_NAME.StakeStoneStakeEth;
const contractType = ContractType.LiquidStaking;

task(DEPLOY_STAKESTONE_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_STAKESTONE_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_STAKESTONE_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
