...
import { WETH, STONE, STAKE_STONE_VAULT } from "./constants";

const contractName: string = CONTRACT_NAME.StakeStoneVaultAdapter;
const contractType = ContractType.StakestoneVault;

task(DEPLOY_STAKESTONE_VAULT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_STAKESTONE_VAULT_ADAPTER);
    }
  });

task(VERIFY_STAKESTONE_VAULT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
