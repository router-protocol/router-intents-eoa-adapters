export const STONE_VAULT: { [chainId: string]: string } = {
  "1": "0xA62F9C5af106FeEE069F38dE51098D9d81B90572",
};

export const STONE: { [chainId: string]: string } = {
  "1": "0x7122985656e38BDC0302Db86685bb972b145bD3C",
  "196": "0x80137510979822322193fc997d400d5a6c747bf7",
};

export const STAKE_STONE_VAULT: { [chainId: string]: string } = {
  "196": "0x57f41EfeaB626B0B529A54532Dc31f4A3A7e0f25",
};

export const WETH: { [chainId: string]: string } = {
  "196": "0x5a77f1443d16ee5761d310e38b62f77f726bc71c",
};

export const SBTC_VAULT: { [chainId: string]: string } = {
  "1": "0x7dBAC0aA440A25D7FB43951f7b178FF7A809108D",
  "56": "0x3aa0670E24Cb122e1d5307Ed74b0c44d619aFF9b",
};

export const BERA_VAULT: { [chainId: string]: string } = {
  "1": "0x8f88aE3798E8fF3D0e0DE7465A0863C9bbB577f0",
};

export const SBTC_LZ_ADAPTER: { [chainId: string]: string } = {
  "1": "0x3f690f43a9fCA689829A22bf925c89B7a48ca57F",
  "56": "0x7122985656e38BDC0302Db86685bb972b145bD3C",
};

export const SBTC: { [chainId: string]: string } = {
  "1": "0x094c0e36210634c3CfA25DC11B96b562E0b07624",
  "56": "0x15469528C11E8Ace863F3F9e5a8329216e33dD7d",
};

export const BERA_STONE: { [chainId: string]: string } = {
  "1": "0x97Ad75064b20fb2B2447feD4fa953bF7F007a706",
};
