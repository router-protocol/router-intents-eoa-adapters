...
import { WETH, STONE } from "./constants";

const contractName: string = CONTRACT_NAME.TokenOracle;
const contractType = ContractType.StakestoneVault;

task(DEPLOY_TOKEN_ORACLE)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_TOKEN_ORACLE);
    }
  });

task(VERIFY_TOKEN_ORACLE).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
