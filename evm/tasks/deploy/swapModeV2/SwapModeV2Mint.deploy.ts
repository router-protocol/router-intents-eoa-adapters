...
import { SWAP_MODE_V2_ROUTER } from "./constants";

const contractName: string = CONTRACT_NAME.SwapModeV2Mint;
const contractType = ContractType.LP;

task(DEPLOY_SWAPMODE_V2_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_SWAPMODE_V2_MINT_ADAPTER);
    }
  });

task(VERIFY_SWAPMODE_V2_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
