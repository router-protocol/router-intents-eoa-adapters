...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_SYMBIOTIC_AFFINE_ETHEREUM_ADAPTER,
  NATIVE,
  VERIFY_SYMBIOTIC_AFFINE_ETHEREUM_ADAPTER,
  WNATIVE,
} from "../../constants";
...
const contractName: string = CONTRACT_NAME.SymbioticAffineEthereum;
const contractType = ContractType.Staking;

task(DEPLOY_SYMBIOTIC_AFFINE_ETHEREUM_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_SYMBIOTIC_AFFINE_ETHEREUM_ADAPTER);
    }
  });

task(VERIFY_SYMBIOTIC_AFFINE_ETHEREUM_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
