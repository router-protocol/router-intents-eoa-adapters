...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_THIRDFY_SWAP_ADAPTER,
  NATIVE,
  VERIFY_THIRDFY_SWAP_ADAPTER,
  WNATIVE,
} from "../../constants";
...
const contractName: string = CONTRACT_NAME.ThirdFySwap;
const contractType = ContractType.Swap;

task(DEPLOY_THIRDFY_SWAP_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_THIRDFY_SWAP_ADAPTER);
    }
  });

task(VERIFY_THIRDFY_SWAP_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
