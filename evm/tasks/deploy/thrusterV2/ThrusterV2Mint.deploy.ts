...
const contractName: string = CONTRACT_NAME.ThrusterV2Mint;
const contractType = ContractType.LP;

task(DEPLOY_THRUSTER_V2_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    const instance = await factory.deploy(
      NATIVE,
      WNATIVE[env][network],
      THRUSTER_V2_ROUTER_FEE_POINT_THREE[network],
      THRUSTER_V2_ROUTER_FEE_ONE[network]
    );
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_THRUSTER_V2_MINT_ADAPTER);
    }
  });

task(VERIFY_THRUSTER_V2_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
