...
import { THRUSTER_NON_FUNGIBLE_POSITION_MANAGER } from "./constants";

const contractName: string = CONTRACT_NAME.ThrusterV3Mint;
const contractType = ContractType.LP;

task(DEPLOY_THRUSTER_V3_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    const instance = await factory.deploy(
      NATIVE,
      WNATIVE[env][network],
      THRUSTER_NON_FUNGIBLE_POSITION_MANAGER[network]
    );
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_THRUSTER_V3_MINT_ADAPTER);
    }
  });

task(VERIFY_THRUSTER_V3_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
