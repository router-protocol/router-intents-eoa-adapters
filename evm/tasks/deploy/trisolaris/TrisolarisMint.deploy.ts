...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_TRISOLARIS_MINT_ADAPTER,
  NATIVE,
  VERIFY_TRISOLARIS_MINT_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { TRISOLARIS_ROUTER } from "./constants";

const contractName: string = CONTRACT_NAME.TrisolarisMint;
const contractType = ContractType.LP;

task(DEPLOY_TRISOLARIS_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_TRISOLARIS_MINT_ADAPTER);
    }
  });

task(VERIFY_TRISOLARIS_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
