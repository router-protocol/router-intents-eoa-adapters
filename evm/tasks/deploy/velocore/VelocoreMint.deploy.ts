...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_VELOCORE_MINT_ADAPTER,
  NATIVE,
  VERIFY_VELOCORE_MINT_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { VELOCORE_TOKEN, VELOCORE_VAULT } from "./constants";

const contractName: string = CONTRACT_NAME.VelocoreMint;
const contractType = ContractType.LP;

task(DEPLOY_VELOCORE_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    const instance = await factory.deploy(
      NATIVE,
      WNATIVE[env][network],
      VELOCORE_TOKEN[network],
      VELOCORE_VAULT[network]
    );
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_VELOCORE_MINT_ADAPTER);
    }
  });

task(VERIFY_VELOCORE_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
