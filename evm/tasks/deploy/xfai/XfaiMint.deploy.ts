...
import {
  CONTRACT_NAME,
  DEFAULT_ENV,
  DEPLOY_XFAI_MINT_ADAPTER,
  NATIVE,
  VERIFY_XFAI_MINT_ADAPTER,
  WNATIVE,
} from "../../constants";
...
import { XFAI_PERIPHERY } from "./constants";

const contractName: string = CONTRACT_NAME.XfaiMint;
const contractType = ContractType.LP;

task(DEPLOY_XFAI_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_XFAI_MINT_ADAPTER);
    }
  });

task(VERIFY_XFAI_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
