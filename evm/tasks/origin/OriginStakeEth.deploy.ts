...
import { ORIGIN_POOL, O_ETH } from "./constants";

const contractName: string = CONTRACT_NAME.OriginStakeEth;

task(DEPLOY_ORIGIN_STAKE_ETH_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_ORIGIN_STAKE_ETH_ADAPTER);
    }
  });

task(VERIFY_ORIGIN_STAKE_ETH_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
