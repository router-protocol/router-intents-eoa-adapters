...
const contractName: string = CONTRACT_NAME.PancakeswapMint;

task(DEPLOY_PANCAKESWAP_MINT_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_PANCAKESWAP_MINT_ADAPTER);
    }
  });

task(VERIFY_PANCAKESWAP_MINT_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
