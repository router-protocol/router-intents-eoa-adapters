...
const NITRO_FACTORY: { [chainId: string]: string } = {
  "59144": "0xAAA32926fcE6bE95ea2c51cB4Fcb60836D320C42",
};
...
export const getPoolData = async (
  ...
  const factoryAddress = NITRO_FACTORY[chainId];
  ...
  const currentPoolAddress = await factory.getPool(
    uniToken0.address,
    uniToken1.address,
    poolFee
  );
...
export const getNileMintData = async (data: {
  user: Wallet;
  chainId: string;
  token0: string;
  token1: string;
