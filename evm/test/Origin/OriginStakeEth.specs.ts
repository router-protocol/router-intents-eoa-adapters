...
describe("OriginStakeEth Adapter: ", async () => {
  ...
  it("Can stake ETH on Origin on dest chain when instruction is received directly on OriginStakeEth adapter", async () => {
    const { originStakeEthAdapter, oEth, mockAssetForwarder } =
      await setupTests();
