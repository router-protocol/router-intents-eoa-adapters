...
import { RedactedStakeEth__factory } from "../../typechain/factories/RedactedStakeEth__factory";
...
describe("RedactedStakeEth Adapter: ", async () => {
  ...
  const setupTests = async () => {
    ...
    const RedactedStakeEth = await ethers.getContractFactory("RedactedStakeEth");
    const redactedStakeEthAdapter = await RedactedStakeEth.deploy(
      NATIVE,
      WNATIVE[env][CHAIN_ID],
      PX_ETH,
      PIREX_POOL
    ...
    await batchTransaction.setAdapterWhitelist(
      [dexSpanAdapter.address, redactedStakeEthAdapter.address],
      [true, true]
    );
    ...
    return {
      ...
      redactedStakeEthAdapter: RedactedStakeEth__factory.connect(
        redactedStakeEthAdapter.address,
        deployer
      ),
  ...
  it("Can stake on redacted on same chain", async () => {
    const { batchTransaction, redactedStakeEthAdapter, pxEth } =
      await setupTests();

    const amount = ethers.utils.parseEther("1");

    const redactedData = defaultAbiCoder.encode(
    ...
    const targets = [redactedStakeEthAdapter.address];
  ...
  it("Can stake ETH on Redacted on dest chain when instruction is received from BatchTransaction contract", async () => {
    const {
      batchTransaction,
      redactedStakeEthAdapter,
      pxEth,
      mockAssetForwarder,
    } = await setupTests();
