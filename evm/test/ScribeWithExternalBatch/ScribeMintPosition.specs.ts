...
describe("ScribeMint Adapter: ", async () => {
  ...
  const setupTests = async () => {
    ...
    const batchTransactionExternal = await BatchTransactionExternal.deploy(
      NATIVE_TOKEN,
      WNATIVE,
    ...
    return {
      batchTransactionExternal: BatchTransactionExternal__factory.connect(
        batchTransactionExternal.address,
        deployer
      ),
  ...
  it("Can mint a new position on Scribe", async () => {
    ...
    const tx = await batchTransactionExternal.executeBatchCallsSameChain(
      0,
      tokens,
      amounts,
      targets,
