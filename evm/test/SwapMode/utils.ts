...
const UNISWAP_V3_FACTORY: { [chainId: string]: string } = {
  "34443": "0x6E36FC34eA123044F278d3a9F3819027B21c9c32",
};
...
export const getPoolData = async (
  ...
  const [tokenADecimals, tokenBDecimals] = await Promise.all([
    tokenAContract.callStatic.decimals(),
    tokenBContract.callStatic.decimals(),
  ]);
  ...
  const currentPoolAddress = await factory.getPool(uniToken0.address, uniToken1.address, poolFee);
...
export const getSwapModeData = async (data: {
  user: Wallet;
  chainId: string;
  token0: string;
  token1: string;
