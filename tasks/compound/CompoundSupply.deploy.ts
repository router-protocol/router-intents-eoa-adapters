...
import {
  ...
  DEPLOY_COMPOUND_SUPPLY_ADAPTER,
  DEXSPAN,
  NATIVE,
  VERIFY_COMPOUND_SUPPLY_ADAPTER,
  WNATIVE,
} from "../constants";
...
const contractName: string = CONTRACT_NAME.CompoundSupply;

task(DEPLOY_COMPOUND_SUPPLY_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_COMPOUND_SUPPLY_ADAPTER);
    }
  });

task(VERIFY_COMPOUND_SUPPLY_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
