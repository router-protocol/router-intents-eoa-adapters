...
import { MATIC, STADER_MATIC_X_TOKEN } from "./constants";

const contractName = CONTRACT_NAME.StaderStakeMatic;

task(DEPLOY_STADER_STAKE_MATIC_ADAPTER)
  ...
  .setAction(async function (
    ...
    if (_taskArguments.verify === true) {
      await _hre.run(VERIFY_STADER_STAKE_MATIC_ADAPTER);
    }
  });

task(VERIFY_STADER_STAKE_MATIC_ADAPTER).setAction(async function (
  _taskArguments: TaskArguments,
  _hre: HardhatRuntimeEnvironment
) {
  let env = process.env.ENV;
