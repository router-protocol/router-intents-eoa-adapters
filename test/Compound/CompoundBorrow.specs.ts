...
describe("CompoundBorrow Adapter: ", async () => {
  ...
  const setupTests = async () => {
    ...
    const CompoundBorrowAdapter = await ethers.getContractFactory("CompoundBorrow");
  ...
  it("Can borrow funds on user's behalf on weth market", async () => {
    ...
    const targets = [compoundSupplyAdapter.address, compoundBorrowAdapter.address];
    const data = [compoundSupplyData, compoundBorrowData];
    const value = [0, 0];
    const callType = [2, 2];

    await compoundWETHPool.allow(
        batchTransaction.address,
        true
    );
  ...
  it("Cannot borrow funds cross-chain when handleMessage is called directly on adapter", async () => {
    const { usdc, mockAssetForwarder, compoundBorrowAdapter } =
      await setupTests();
